function rgb_dem = demosaicking_InnerInter(img)

%%
tic;
fprintf( sprintf('\nRunning demosaick MyCombined ... \n'));
rgb = double(img);
A = img;
[M,N,ch] = size(A);
...
%%
inter = gree(:,:);
inner= demosaicking_Inpainting(img);
% h(11:M-10,11:N-10,2)=0.9*inter(11:M-10,11:N-10)+0.1*inner;
lambda=0.55;    %For Imax 0.55; For Kodak 0.9;
h(11:M-10,11:N-10,2)=lambda*inter(11:M-10,11:N-10)+(1-lambda)*inner;
%%
...
% % figure,imshow(uint8(rgb_dem(350:450,200:380,:)));
% figure,imshow(uint8(rgb_dem));
% imwrite(uint8(rgb_dem),'B_my.png');
% % figure,imshow(uint8(rgb_dem(100:800,1100:1400,:)));